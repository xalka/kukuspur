...
<div class="w-full">
    ...
    <div class="flex flex-row justify-between mb-2">
        ...
        <div class="flex gap-2">
            ...
            <button class="btn btn-create group" data-modal-href="/user?view=modal&action=create" data-title="user">
...
<div class="table-div">
    <table>
        ...
        <?php foreach ($data['users'] as $user) { ?>
            <tr class="bg-gray-50">
                <td class="text-capitalize"><?=!empty($user['cname'])?$user['cname']:$user['fname'].' '.$user['lname']?></td>
                <td><?=!empty($user['phone'])?$user['phone']:'-'?></td>
                <td class="text-lowercase"><?=$user['email']?></td>
                <td class="text-capitalize"><?=$user['role']?></td>
                <td>
                    <span class="btn btn-<?=$user['active']?'approved':'rejected'?>"><?=$user['active']?'active':'in-active'?></span>
                </td>
                <td><?=$user['date']?></td>
                <td><?=$user['time']?></td>
                    ...
                    <div class="inline-block relative group">
                        ...
                        <div class="action-dropdown group-hover:visible group-hover:opacity-100">
                            <ul class="py-1 text-left">
                                <li class="view" data-modal-href="/user?view=modal&action=view&id=<?=$user['_id']?>" data-title="view">View</li>
                                <li class="update" data-modal-href="/user?view=modal&action=edit&id=<?=$user['_id']?>" data-title="update">Update</li>
                                <li class="suspend" data-modal-href="/user?view=modal&action=suspend&id=<?=$user['_id']?>&names=<?=$user['fname'].' '.$user['lname']?>" data-title="suspend">Suspend</li>
                            </ul>
                        </div>
                    </div>
                </td>
